...
def preprocess_english_data(cfg):
    ...
    def _vectorize_split(split_df, split_name):
        ...
        tokenizer = AutoTokenizer.from_pretrained(cfg.embedding.path)
        X_split = tokenizer(split_df[cfg.pre_processing.token_type].tolist(), padding="max_length", truncation=True)
...
def preprocess_and_store_splits(cfg, dev_set, test_set, annotator_names, fold=None):
    ...
    for i, split in enumerate([dev_set, test_set]):
        ...
        X_split = split[cfg.pre_processing.token_type]
        ...
        if cfg.embedding.name != "transformer":
            ...
            tokenizer = AutoTokenizer.from_pretrained(cfg.embedding.path)
            X_split = tokenizer(X_split.tolist(), padding="max_length", truncation=True)
